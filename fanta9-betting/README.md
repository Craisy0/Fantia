...
- le giornate **non ancora giocate** vengono estratte a ogni run da probabilità 1X2 calcolate con un **modello a livello giocatore** (diverso dalla proiezione FantaLab usata per i singoli testa a testa settimanali, che restano invariati): per ogni giornata, la forza di una squadra GOTTA è la somma della quotazione Mantra dei suoi migliori 11 giocatori (1 portiere + 10 di movimento) tra quelli che risultano titolari reali nel proprio club di Serie A, ciascuno pesato da un fattore che premia/penalizza in base a quanto è forte o debole l'avversario reale che affronta quella giornata (attacco avversario debole aiuta i difensori, difesa avversaria debole aiuta gli attaccanti), smorzato verso la media di lega quando l'avversario ha giocato poche partite reali (dato ancora rumoroso). I dati sono **statici**: formazioni titolari reali e listone congelati allo scatto della stagione (`data/rose_gotta.json`, `data/titolari_reali.json`), calendario reale di Serie A giornate 4-22 (`data/calendario_serie_a.json`) e statistiche gol fatti/subiti (`data/stats_serie_a.json`, con smorzamento regolabile da `config.json` → `girone_andata.smorzamento_partite`) — vanno aggiornati a mano se si vogliono rinfrescare (es. dopo il calciomercato o con più giornate di Serie A giocate);
...
La frequenza con cui ogni squadra risulta prima in classifica nelle simulazioni diventa la sua probabilità di vincere il girone, convertita in quota con lo stesso margine bookmaker delle altre quote, ma con una tassa/tetto dedicati e più larghi (`config.json` → `girone_andata.quota_soglia_tassa`/`quota_massima`/`quota_tassa_scala`, di default fino a **30.00**): essendo un mercato a 10 vie le favorite possono staccare molto di più che in un singolo testa a testa. Il risultato è puramente indicativo finché mancano molte giornate (a inizio stagione, con poche proiezioni disponibili, le quote di tutte le squadre saranno simili) e si affina man mano che importi punteggi e proiezioni reali.

Premendo "Pubblica come scommessa" il sistema crea una scommessa libera dedicata (un esito `Vince <squadra>` per ognuna delle 10), non legata a una giornata specifica: si gioca da sola, non si può combinare in una schedina con altri mercati, e **si può scegliere un solo vincitore** — una volta giocata la selezione resta bloccata (l'admin non può pubblicarne una seconda finché quella aperta non viene chiusa o risolta). Ogni volta che importi i punteggi di una giornata entro la fine del girone d'andata, le quote di questo mercato **si ricalcolano automaticamente** con i dati aggiornati; le schedine già giocate non ne risentono, perché ciascuna si porta dietro la quota congelata al momento della giocata. Per liquidarla a fine girone la risolvi come una qualunque scommessa libera scegliendo la squadra vincitrice.
...
### Scheda squadra (statistiche pubbliche)

Nel tab "Classifica", toccando una qualsiasi squadra si apre la sua scheda completa, consultabile da chiunque abbia scelto un'identità (non solo dall'admin o dalla squadra stessa): saldo attuale, conteggio vinte/perse/in corso, quota media giocata, totale puntato, un grafico dell'andamento del saldo nel tempo e lo storico completo di tutte le schedine giocate. L'andamento del saldo è ricostruito dagli eventi reali (ogni schedina piazzata/vinta, ogni correzione saldo dell'admin), quindi è accurato fin dall'inizio della lega, non solo da quando esiste questa funzione.
...
Nel tab Admin → "Notifiche push" trovi due scorciatoie che avvisano solo chi non ha ancora giocato — "Ricorda schedina (ultima giornata)" e "Ricorda vincitore girone d'andata" — più un campo libero per un messaggio a piacere a tutti. Chi non ha attivato le notifiche semplicemente non riceve nulla (nessun errore, il sistema salta chi non ha sottoscrizioni attive). Inoltre, appena liquidi una giornata (o risolvi a mano un mercato), ogni squadra coinvolta riceve subito una notifica automatica con l'esito della propria schedina (vinta con l'importo, o persa) — non serve fare nulla, parte da sola.
